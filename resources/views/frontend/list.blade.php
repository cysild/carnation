...
<section class="used-ferrari main-section fl w100">
	<div class="container">
		<p class="text-right bread-crumb">Home/
			@if($ctype)
			<span class="navigation-color">{{ucfirst($ctype)}}@if($cmake) / {{ucfirst($cmake)}} @endif</span>
			@endif
		...
		<div class="row">
			<div class="col-lg-3 col-md-3 col-sm-3 col-xs-12 filter-padding">
				<div class="filter fl w100">
					...
					<form name="filter-form">
						<select id="make" >
							<option value="{{url('/'.$ctype)}}">Select model</option>
							@foreach($make as $make)
							<option value="{{url('/'.$ctype)}}/{{str_slug($make->make)}}"  @if($cmake == str_slug($make->make)) selected @endif>{{$make->make}}</option>
							@endforeach
						</select>
					</form>
				</div>
			...
			<div class="col-lg-9 col-md-9 col-sm-9 col-xs-12">
				<h3 class="avenir-demi">Car Type <span class="avenir-light">{{ucfirst($ctype)}}</span>
				</h3>
	...
	<div class="container">
		<div class="row">
			...
			<div class="col-lg-9 col-md-9 col-sm-9 col-xs-12 about-make-section">
				<h3 class="avenir-demi">About <span class="avenir-light">{{$ctype}}</span></h3>
				<div class="border-bm"></div>
				{!!$desc!!}
			</div>
		</div>
